...
import { ModaicClientDefaultError } from "../models/errors/modaic-client-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | ModaicClientDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | ModaicClientDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ModaicClientDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ModaicClientDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
